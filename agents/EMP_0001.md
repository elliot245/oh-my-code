  ...
  - name: watchdog
    ...
    task: |
      ...
      1) Identify the Team Lead + its tmux session:
         ...
         - `tmux list-sessions`
         - If the Team Lead agent is stopped/unresponsive, start it:
      ...
      3) If Team Lead looks stuck/idle >15m:
         ...
         - If you restarted the Team Lead, explicitly tell it to resume the repo workflow:
           - `python3 .claude/skills/agent-manager/scripts/main.py assign coder-a <<'EOF'`
             Follow the workflow in `workflows/github_issues.md` (use `TEAM.md` to confirm roles).
             Objective: pick the next issue assigned to @me and start it end-to-end.
             Done: post back a short update with Summary/Evidence/Files Changed/Risks/Next Step.
             EOF
